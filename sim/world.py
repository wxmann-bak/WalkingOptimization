...
class StreetGrid(object):
    ...
    def get_NS_st(self, n):
        if n > self.width():
            raise ValueError("Requested street {0} greater than number of streets {1}".format(n, self.width()))
        return self.sts_NS[n-1]

    def get_EW_st(self, n):
        if n > self.height():
            raise ValueError("Requested street {0} greater than number of streets {1}".format(n, self.height()))
